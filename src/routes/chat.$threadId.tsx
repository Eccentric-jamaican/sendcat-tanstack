...
function ChatPage() {
  ...
  const handleRetry = async (userMessageId: string, modelId?: string) => {
    ...
    try {
      ...
      const selectedModel =
        modelId || localStorage.getItem('t3_selected_model') || 'google/gemini-2.0-flash-exp:free'
      ...
      const reasoningEffort =
        (supportsEffortReasoning || supportsMaxTokensReasoning) && savedReasoning
  ...
  return (
    <div className="bg-background text-foreground relative flex h-dvh min-h-screen max-w-full overflow-hidden">
        ...
        >
          {isEmpty ? (
            <div className="flex h-full w-full flex-col items-center justify-center">
              <LandingHero
                onSelectPrompt={(text) => chatInputRef.current?.setContentAndSend(text)}
              />
            </div>
          ) : (
                            ...
                            <motion.div
                              ...
                              className={cn(
                                ...
                                msg.role === 'user'
                                  ...
                                  : 'text-foreground/90 w-full max-w-none px-4 py-1 md:px-2',
                              )}
                            >
                                ...
                                {msg.role === 'assistant' ? (
                                  <>
                                      ...
                                      <div className="mb-2 flex flex-col gap-2">
                                        {msg.toolCalls.map((tc: any, i: number) => {
                                          ...
                                          return (
                                            <div
                                              key={i}
                                              className="text-foreground/70 flex flex-col gap-1 rounded border border-black/5 bg-black/5 p-2 font-mono text-xs"
                                            >
                                              ...
                                              {toolMsg && (
                                                <div className="text-foreground/50 line-clamp-2 pl-4 text-[11px] whitespace-pre-wrap italic">
                                                  {toolMsg.content}
                                                </div>
                                              )}
                                            </div>
                                    ...
                                    {msg.status === 'streaming' &&
                                      !msg.content.trim() &&
                                      !msg.toolCalls && (
                                        <div className="text-foreground/60 flex items-center gap-2.5 py-2">
                                          <motion.div
                                          ...
                                          >
                                            {[0, 1, 2].map((i) => (
                                              <motion.span
                                                key={i}
                                                className="from-t3-berry to-t3-berry-deep h-2 w-2 rounded-full bg-gradient-to-br shadow-sm"
                                                animate={{
                                                  y: [0, -6, 0],
                                                  scale: [1, 1.15, 1],
                                                  opacity: [0.7, 1, 0.7],
                                  ...
                                  msg.name === 'search_web' || msg.name === 'search_ebay' ? null : (
                                    <div className="text-foreground/60 max-h-32 overflow-y-auto rounded border border-black/5 bg-black/5 p-2 font-mono text-xs whitespace-pre-wrap">
                        ...
                        >
                          {msg.role === 'user' ? (
                            <>
                              ...
                              >
                                <button className="text-foreground/40 hover:text-foreground/70 flex items-center justify-center rounded-md p-1.5 transition-all hover:bg-black/5">
                                  <RotateCcw size={15} />
                                </button>
                              </MessageActionMenu>
                              <MessageActionMenu
                              ...
                              <ActionButton
                                icon={
                                  copiedId === msg._id ? <Check size={15} /> : <Copy size={15} />
                                }
                                label="Copy"
                            ...
                            <>
                              ...
                              <MessageActionMenu
                                ...
                                onAction={(modelId) => {
                                  ...
                                  const msgIndex =
                                    messages?.findIndex((m: any) => m._id === msg._id) ?? -1
                                  if (msgIndex > 0) {
                              ...
                              <MessageMetadata
                                ...
                                wordCount={
                                  msg.content?.trim().split(/\s+/).filter(Boolean).length ?? 0
                                }
...
const ActionButton = ({
  ...
  <Tooltip>
    <TooltipTrigger asChild>
      <button
        onClick={onClick}
        className="text-foreground/60 hover:text-foreground/80 flex items-center justify-center rounded-md p-1.5 transition-all hover:bg-black/5"
      >
        {icon}
      </button>
    </TooltipTrigger>
